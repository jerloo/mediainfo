...
func GetVideoCreationTimeMetadataFromFile(filePath string) (time.Time, error) {
	videoBuffer, err := os.Open(filePath)
	if err != nil {
		return time.Time{}, err
	}
	defer videoBuffer.Close()

	return GetVideoCreationTimeMetadata(videoBuffer)
}
